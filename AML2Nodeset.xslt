...
<xsl:stylesheet 
	...
	exclude-result-prefixes="#default caex xsi xsl exslt fn ua fn2">
		...
		<Models>
			<xsl:for-each select="*[local-name()='CAEXFile']/*[local-name()='InstanceHierarchy' or local-name()='InterfaceClassLib' or local-name()='RoleClassLib' or local-name()='SystemUnitClassLib' or local-name()='AttributeTypeLib']">
				<Model>
					...
					<xsl:attribute name="Version" select="*[fn:local-name()='Version']"/>
		...
		<UANodeSet 
			...
			<UAObject>
				...
				<References>
					...
					<xsl:if test="//*[local-name()='InterfaceClassLib' and @Name!='AutomationMLInterfaceClassLib']">
						<Reference ReferenceType="Organizes">
							<xsl:call-template name="FormatRef">
					...
					<xsl:if test="//*[local-name()='RoleClassLib' and @Name!='AutomationMLBaseRoleClassLib']">
						<Reference ReferenceType="Organizes">
							<xsl:call-template name="FormatRef">
					...
					<xsl:if test="//*[local-name()='AttributeTypeLib' and @Name!='AutomationMLBaseAttributeTypeLib']">
						<Reference ReferenceType="Organizes">
							<xsl:call-template name="FormatRef">
...
****************************************************************************** -->
	...
	<xsl:template name="GetObjectName">
		...
		<xsl:choose>
			<xsl:when test="$Object/@Name != '' and (local-name($Object)='SystemUnitClass' or local-name($Object)='RoleClass' or local-name($Object)='AttributeTypeClass' or local-name($Object)='InterfaceClass')">
	...
	<xsl:template match="*[local-name()='Attribute']">
		<xsl:if test="fn:local-name(parent::node()) != 'RoleRequirements'">
			<xsl:variable name="NsId">
				<xsl:call-template name="GetNamespaceId"/>
			</xsl:variable>
			<!-- Get object variables -->
			...
			<xsl:variable name="AttributeDataType">
				<xsl:value-of select="translate(@AttributeDataType, 'ABCDEFGHIJKLMNOPQRSTUVWXYZ', 'abcdefghijklmnopqrstuvwxyz')"/>
			</xsl:variable>
			<xsl:variable name="dtList">
				<xsl:copy-of select="exslt:node-set($DataTypes)/*[translate(@AML, 'ABCDEFGHIJKLMNOPQRSTUVWXYZ', 'abcdefghijklmnopqrstuvwxyz')=$AttributeDataType]"/>
			</xsl:variable>
				...
				<xsl:choose>
					<xsl:when test="$Datatype='String' or $Datatype='Boolean' or contains($Datatype, 'Int')">
						<xsl:choose>
							...
							<xsl:when test="*[fn:local-name()='DefaultValue']!=''">
								<Value>
									<xsl:element name="{concat('uax:',$Datatype)}">
										<xsl:value-of select="*[fn:local-name()='DefaultValue']"/>
									</xsl:element>
								</Value>
							</xsl:when>
							<xsl:otherwise>
	...
	<xsl:template match="*[local-name()='RefSemantic']">
		...
		<xsl:comment>
			<xsl:value-of select="concat('RefSemantic: ', $ParentClearName, 'RefSemantic')"/>
		</xsl:comment>
		<UAVariable>
			<xsl:attribute name="NodeId">
				<xsl:call-template name="FormatRef">
			...
			<xsl:attribute name="BrowseName">
				<xsl:value-of select="concat($NsId, ':', $ObjectId, '_RefSemantic')"/>
			</xsl:attribute>
			<xsl:attribute name="ParentNodeId">
				<xsl:call-template name="FormatRef">
					<xsl:with-param name="ObjectId" select="$ObjectId"/>
			...
			<References>
				...
				<!--xsl:comment>HasTypeDefinition: AMLBaseVariableType</xsl:comment-->
				<Reference ReferenceType="HasTypeDefinition">AMLBaseVariableType</Reference>
			</References>
			<Value>					
				<uax:String>
	...
	<xsl:template name="References">
		...
		<xsl:for-each select="*/*[fn:local-name()='AMLRequiredValue' or fn:local-name()='AMLRequiredMinValue' or fn:local-name()='AMLRequiredMaxValue']">
			...
			<Reference ReferenceType="HasComponnent">
				<xsl:call-template name="FormatRef">
					<xsl:with-param name="ObjectId" select="concat($ObjectId, '_', $AttributeName)"/>
					<xsl:with-param name="Namespace" select="$Namespace"/>
				</xsl:call-template>
		...
		<!-- References for all RefSematic properties -->
		...
		<xsl:choose>
			<xsl:when test="@RefBaseSystemUnitPath">
				<xsl:variable name="path" select="@RefBaseSystemUnitPath"/>
				<!-- Parse SupportedRoleClass -->
				<xsl:variable name="SUCLibName" select="substring-before(@RefBaseSystemUnitPath,'/')"/>
				<xsl:variable name="SUCAliasName" select="substring-after(@RefBaseSystemUnitPath,'@')"/>
				...
				<xsl:choose>
					...
					<xsl:when test="contains($SUCLibName, '@')">
						<Reference ReferenceType="HasTypeDefinition">
							<xsl:value-of select="$SUCAliasName"/>
						</Reference>
					</xsl:when>
		...
		<xsl:for-each select="*[local-name()='SupportedRoleClass' or fn:local-name()='RoleRequirements']">
			<!-- Parse SupportedRoleClass -->
			<xsl:variable name="RCLibName" select="concat(substring-before(@RefBaseRoleClassPath,'/'), substring-before(@RefRoleClassPath,'/'))"/>
			<xsl:variable name="RCAliasName" select="concat(substring-after(@RefBaseRoleClassPath,'@'), substring-after(@RefRoleClassPath,'@'))"/>
			<xsl:variable name="RCContent">
				<xsl:if test="$RCLibName!= '' and @RefRoleClassPath!= 'AutomationMLBaseRoleClassLib/AutomationMLBaseRole'">
					<xsl:call-template name="GetClass">
						<xsl:with-param name="path" select="@RefRoleClassPath"/>
					</xsl:call-template>
				</xsl:if>
				<xsl:if test="$RCLibName!= '' and @RefBaseRoleClassPath!= 'AutomationMLBaseRoleClassLib/AutomationMLBaseRole'">
					<xsl:call-template name="GetClass">
						<xsl:with-param name="path" select="@RefBaseRoleClassPath"/>
					</xsl:call-template>
				</xsl:if>
			</xsl:variable>
			...
			<xsl:choose>
				...
				<xsl:when test="contains($RCLibName, '@')">
					<Reference ReferenceType="HasAMLRoleReference">
						<xsl:value-of select="$RCAliasName"/>
					</Reference>
				</xsl:when>
			</xsl:choose>
		</xsl:for-each>
...
****************************************************************************** -->
	...
	<xsl:template match="*[fn:local-name()='ExternalInterface']">
		...
		<xsl:variable name="ICAliasName" select="substring-after(@RefBaseClassPath,'@')"/>
		<xsl:variable name="ICLibName" select="fn:substring-before(@RefBaseClassPath,'/')"/>
		<xsl:variable name="ICContent">
			<xsl:if test="$ICLibName!= '' and @RefBaseClassPath!= 'AutomationMLInterfaceClassLib/AutomationMLBaseInterface'">
				<xsl:call-template name="GetClass">
		...
		<UAObject>
			...
			<References>
				...
				<xsl:choose>
					...
					<xsl:when test="contains($ICLibName, '@')">
						<Reference ReferenceType="HasTypeDefinition">
							<xsl:value-of select="$ICAliasName"/>
						</Reference>
					</xsl:when>
				</xsl:choose>
				<xsl:call-template name="References">
